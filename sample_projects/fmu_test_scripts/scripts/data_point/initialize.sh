...
if [[ (-z $1) || (-z $2) ]]; then
    echo "Expecting script to have 2 parameters:"
    echo "  1: Name of the exiting gem to replace, e.g. openstudio-standards"
    echo "  2: file location of new gem"
    echo "  -- example use: ./set_standards_version.sh /usr/local/openstudio-2.7.1/Ruby openstudio-standards NREL/openstudio-standards"
    exit 1
fi
...
function replace_gem_in_files (){
  ...
  if [ -d "$filepath/clones/$gem_name" ]; then
    rm -rf $filepath/clones/$gem_name
  fi
  cp -r /mnt/openstudio/analysis_$SCRIPT_ANALYSIS_ID/lib/$gem_path $filepath/clones/$gem_name

  echo "***Replacing gem: $gem_name with version at $gem_path"
